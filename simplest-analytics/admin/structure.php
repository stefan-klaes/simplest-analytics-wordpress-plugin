 ...
 */
	...
	<div class="wrap" style="max-width:1000px">
		...
		<nav class="nav-tab-wrapper woo-nav-tab-wrapper" style="margin-bottom:10px;">
			...
			<a id="database" onclick="simplest_analytics_toggle_tabs_by_id(this)" class="nav-tab <?php echo $tab=="url_paras" ? '' : 'nav-tab-active' ?>">
		...
		?>
		<div id="tab_database" class="all_tabs" <?php echo $tab=="url_paras" ? ' style="display:none"' : '' ?>>
		<form method="post" action="">
		<table class="settings_table">
		<tr class="head_th">
				<th></th>
				<th><?php echo __( 'value', 'simplest-analytics' ) ?></th>
				<th><?php echo __( 'info', 'simplest-analytics' ) ?></th>
			</tr>
			
			<tr>
				<th><?php echo __( 'Database Table name', 'simplest-analytics' ) ?></th>
				<td>
			...
			<tr>
				<th><?php echo __( 'Total entries', 'simplest-analytics' ) ?></th>
				<td>
					<?php echo esc_attr( $total_entries ) ?>
				</td>
				<td>
				<?php echo __( 'number of values in the database table', 'simplest-analytics' ) ?>: <?php echo esc_html( $table_name ) ?>
				</td>
			</tr>

			<tr>
				<th><?php echo __( 'clear options', 'simplest-analytics' ) ?></th>
				<td>
					<div id="all" class="button button-primary simplest_analytics_clear_ele">
						<?php echo __( 'delete all entries', 'simplest-analytics' ) ?> (<?php echo esc_attr( $total_entries ) ?>)
					</div>
				</td>
				<td>
				...
				<tr>
					...
					<td>
						<div id="older_than_year" class="button button-primary simplest_analytics_clear_ele">
							<?php echo __( 'delete entries older than this year', 'simplest-analytics' ) ?> (<?php echo esc_attr( $total_entries_not_current_year ) ?>)
						</div>
					</td>
					<td>
					<?php echo __( 'All entries will be deleted that are not from', 'simplest-analytics' ) ?> <?php echo esc_attr( $current_year ) ?>.
					</td>
				</tr>
				<?php
			}
			...
			<tr>
				<th><?php echo __( 'clear options', 'simplest-analytics' ) ?></th>
				<td>
					<input type="date" id="delete_before_date" style="max-width: 200px;margin-bottom: 10px; display: block"/>
		...
		<div id="simplest_analytics_loading" style="display: none">
			<div style="background: #000; position: fixed; top: 0px; left: 0px; right: 0px; bottom: 0px; opacity: 0.5; z-index: 999"></div>
			<!-- dentered loading text fixed -->
			<div style="background: #fff; position: fixed; top: 50%; left: 50%; margin-top: -50px; margin-left: -100px; width: 200px; height: 100px; z-index: 1000" id="simplest_analytics_loading_text">
